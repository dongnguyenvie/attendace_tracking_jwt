...
public class JwtService {
	...
	public static final String SECRET_KEY = "11111111111111111111softlink11111111111111111111";
	...
	public String generateTokenLogin(User user) {
		...
		try {
			...
			//builder.claim(USERNAME, user.getUs_username());
			builder.claim(ID, user.getUs_id());
	...
	public int getIDUserFromToken(String token) {
		...
		try {
			JWTClaimsSet claims = getClaimsFromToken(token);
			idUser = claims.getIntegerClaim(ID);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return idUser;
	...
	public Boolean validateTokenLogin(String token) {
		...
		int id = getIDUserFromToken(token);
		if (id == 0 ) {
			return false;
		}
